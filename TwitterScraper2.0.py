...
class TwitterSearch(metaclass=ABCMeta):
    ...
    @staticmethod
    #how do I make this function work when it's inside (dependent?) on the other
    ...
    @staticmethod
    #what I want my url to look like: https://twitter.com/search?f=tweets&vertical=default&q=from%3Akimkardashian%20since%3A2006-03-21%20until%3A2017-01-24&src=typd
    ...
    @abstractmethod
    #is the idea to fill this out with my fileIO code? write to text file as JSON or whatever?
...
class JSONTwitterSlicer(TwitterSlicer):
    ...
    def save_tweets(self, tweets):
        ...
        filename = 'output-{}.json'.format(thread.ident)
...
if __name__ == '__main__':
    ...
    #the typical query looks like this
    #search_query = "Kardashian"

    #all you have to do to get the right string is to translate the url characters '%3' etc to regular string
    search_query = 'from:kuwtk'
...
##    twit.search(search_query)
    ...
    select_tweets_since = datetime.datetime.strptime("2006-12-01", '%Y-%m-%d')
    select_tweets_until = datetime.datetime.strptime("2017-1-26", '%Y-%m-%d')
    threads = 10

    twitSlice = JSONTwitterSlicer(rate_delay_seconds, error_delay_seconds, select_tweets_since, select_tweets_until,
                              threads)
